...
fn read(base: &str, attr: &str) -> Option<String> {
    std::fs::read_to_string(format!("{base}/{attr}")).ok().map(|s| s.trim().to_string())
}
...
impl BatteryEngine {
    pub fn new() -> Self {
        ...
        Self { history_path: dir.join("battery-history.json") }
    }
    ...
    fn record(&self, sample: &BatterySample) {
        ...
        let due = hist.last().map_or(true, |l| sample.ts.saturating_sub(l.ts) > 6 * 3600 * 1000);
        if !due {
            return;
        }
    ...
    pub fn report(&self) -> Option<BatteryReport> {
        ...
        let capacity = read(base, "capacity").and_then(|s| s.parse().ok()).unwrap_or(0.0);
        let cycle_count = read_u64(base, "cycle_count").unwrap_or(0) as u32;
        let voltage_v = read_u64(base, "voltage_now").map(|v| v as f32 / 1e6).unwrap_or(0.0);
        let voltage_min = read_u64(base, "voltage_min_design").map(|v| v as f32 / 1e6).unwrap_or(0.0);
        let power_w = read_u64(base, "power_now").map(|p| p as f32 / 1e6).unwrap_or(0.0);
        ...
        Some(BatteryReport {
            ...
            discharge_rate_w: if !charging && !status.eq_ignore_ascii_case("Full") { power_w } else { 0.0 },
            runtime_min,
            lifespan,
            degradation,
            recommendations,
    ...
    pub fn export_markdown(&self) -> Option<String> {
        ...
        s.push_str(&format!("- Health: {:.1}%  ·  Wear: {:.1}%\n", r.health_percent, r.wear_percent));
        s.push_str(&format!("- Capacity: {:.1} Wh full / {:.1} Wh design\n", r.full_wh, r.design_wh));
        s.push_str(&format!("- Cycles: {}\n", r.cycle_count));
        s.push_str(&format!("- Voltage: {:.2} V (min design {:.2} V)\n\n", r.voltage_v, r.voltage_min_design_v));
        s.push_str("## Lifespan\n\n");
        s.push_str(&format!("- {}\n", r.lifespan.summary));
        s.push_str(&format!("- Equivalent cycles consumed: ~{}\n\n", r.lifespan.equivalent_cycles));
...
fn rec(severity: &str, title: &str, detail: &str) -> BatteryRecommendation {
    BatteryRecommendation { severity: severity.into(), title: title.into(), detail: detail.into() }
}

fn recommendations(health: f32, capacity: f32, cycles: u32, status: &str, charging: bool) -> Vec<BatteryRecommendation> {
    let mut out = Vec::new();

    if health < 60.0 {
        out.push(rec("critical", "Significant battery wear", "Health is below 60%. Consider a battery replacement for full runtime."));
    } else if health < 80.0 {
        out.push(rec("warning", "Battery aging", "Health is below 80%. Set an 80% charge limit to slow further wear."));
    }

    if capacity >= 99.0 && (status.eq_ignore_ascii_case("Full") || charging) {
        out.push(rec("info", "Avoid sitting at 100%", "Keeping Li-ion at full charge accelerates wear. An 80% charge cap improves longevity."));
    }

    if cycles > 800 {
        out.push(rec("warning", "High cycle count", "Over 800 charge cycles. Expect gradual capacity loss; consider power-saver habits."));
    }

    if !charging && status.eq_ignore_ascii_case("Discharging") {
        out.push(rec("info", "On battery", "Enable Battery Saver to extend runtime and reduce heat."));
    }

    if out.is_empty() {
        out.push(rec("info", "Battery healthy", "No action needed. Occasional full discharges help calibrate the gauge."));
    }
    out
}
