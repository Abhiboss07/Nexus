...
pub fn predict(
    ...
    if let Some(b) = battery {
        ...
        let sev = if b.health_percent < 70.0 { "warning" } else { "info" };
    ...
    if let Some(disk) = snapshot.storage.iter().max_by(|a, b| a.usage.total_cmp(&b.usage)) {
        if disk.usage >= 75.0 {
            out.push(MaintenanceInsight {
                ...
                prediction: format!("{} is {:.0}% full — plan a cleanup before performance degrades.", disk.mount_point, disk.usage),
                eta_days: None, // no fill-rate history yet to project an ETA
                confidence: (60.0 + (disk.usage - 75.0)).clamp(0.0, 95.0).round() as u8,
                severity: if disk.usage >= 90.0 { "warning" } else { "info" }.into(),
                evidence: vec![Evidence::new(&format!("{} used", disk.mount_point), format!("{:.0}%", disk.usage), "90%")],
            });
        }
        if disk.smart_status == "failing" {
            out.push(MaintenanceInsight {
                component: "Storage".into(),
                prediction: format!("{} reports a failing SMART status — back up and replace the drive soon.", disk.device),
                eta_days: None,
                confidence: 95,
    ...
    let cpu_temp = trends::analyze(history).metrics.into_iter().find(|m| m.metric == "CPU Temp");
...
mod tests {
    ...
    fn flags_failing_smart_critical() {
        ...
        s.storage.push(StorageTelemetry { device: "nvme0".into(), mount_point: "/".into(), filesystem: "btrfs".into(), total_bytes: 1, used_bytes: 1, usage: 50.0, temperature_c: None, read_bytes_sec: 0, write_bytes_sec: 0, smart_status: "failing".into() });
    ...
    fn battery_report_stub() -> BatteryReport {
        ...
        BatteryReport {
            present: true, status: "full".into(), capacity_level: "Full".into(), technology: "Li-ion".into(),
            manufacturer: "x".into(), model: "y".into(), serial: "z".into(),
            charge_percent: 100.0, health_percent: 85.0, wear_percent: 15.0, score: 86, grade: BatteryGrade::Good,
            design_wh: 83.0, full_wh: 71.0, now_wh: 71.0, voltage_v: 12.7, voltage_min_design_v: 11.5,
            cycle_count: 0, charging: false, power_draw_w: 0.0, charge_rate_w: 0.0, discharge_rate_w: 0.0,
            runtime_min: None,
            lifespan: LifespanEstimate { equivalent_cycles: 356, cycles_to_eol: 144, years_remaining: 0.4, summary: "x".into() },
            degradation: crate::control::battery::engine::DegradationTrend { samples: 1, first_full_wh: 71.0, current_full_wh: 71.0, lost_wh: 0.0, span_days: 0.0 },
            recommendations: vec![],
        }
    }
}
