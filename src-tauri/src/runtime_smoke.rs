...
fn service_builds_and_reports_compatibility() {
    ...
    match report.tier {
        SupportTier::Unsupported | SupportTier::Unknown => {
            assert!(!report.fan_writes, "unvalidated hardware must not enable fan writes");
            assert!(!report.rgb_writes, "unvalidated hardware must not enable rgb writes");
        }
...
fn capabilities_respect_the_safety_gate() {
    ...
    if !report.fan_writes {
        assert!(!caps.fan.status.controllable, "fan control must be hidden when gate denies writes");
    }
    if !report.rgb_writes {
        assert!(!caps.rgb.status.controllable, "rgb control must be hidden when gate denies writes");
    }
}
...
fn process_scanner_returns_live_rows() {
    ...
    assert!(!rows.is_empty(), "/proc scan should see at least this test process");
    for r in &rows {
        assert!(r.pid > 0);
