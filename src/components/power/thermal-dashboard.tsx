...
import { useFans, useThermals, useHistory } from "@/hooks/use-telemetry";
...
export function ThermalDashboard() {
  ...
  const cpuC = thermals?.cpuC ?? thermal?.cpuC ?? null;
  const gpuC = thermals?.gpuC ?? thermal?.gpuC ?? null;

  const trend = history.slice(-60).map((p, i) => ({
    i,
  ...
  return (
    <motion.div variants={stagger(0.05)} initial="hidden" animate="show" className="space-y-md">
      {/* Top row: fan gauges + thermal score */}
      ...
      <div className="grid grid-cols-1 gap-md lg:grid-cols-3">
        ...
        <motion.div variants={fadeUp}>
          <GlassCard padding="lg" className="h-full">
              ...
              <div className="h-44 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={curve} margin={{ top: 6, right: 8, bottom: 0, left: -24 }}>
                    ...
                    {cpuC != null && <ReferenceLine x={Math.round(cpuC)} stroke="rgb(var(--color-danger))" strokeDasharray="3 3" />}
            ...
            <div className="mt-sm">
              <StatRow label="Operating point" value={`${cpuC?.toFixed(0) ?? "—"}°C @ ${cpuRpm} rpm`} />
